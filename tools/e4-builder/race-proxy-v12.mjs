...
const selfTestFlag = process.argv.includes("--self-test-direct-copy")
  ? "--self-test-direct-copy"
  : (process.argv.includes("--self-test") ? "--self-test" : null);

if (selfTestFlag) {
  const child = spawn(process.execPath, ["--import", preload, daemon, selfTestFlag], {
    cwd: here,
    env: process.env,
    stdio: "inherit",
  });
  ...
  input.on("line", (line) => {
    ...
    const item = {
      ...
      timer: setTimeout(() => {
        ...
        item.emitted = true;
        process.stdout.write(JSON.stringify({ok: false, error: "E4 V12 builder race timed out"}) + "\n");
        pending.delete(id);

...
function quoteExactSolIn(request, metadata) {
  const spendableSol = positiveBigInt(Number(request.amount) * 1_000_000_000, "exact SOL input");
  const fees = feeBps(metadata);
  let netSol = (spendableSol * 10_000n) / (10_000n + fees);
  if (netSol > 0n) netSol -= 1n;
  const virtualSol = reserveLamports(metadata.virtual_sol_reserves, "virtual SOL reserves");
  const virtualTokens = reserveTokens(metadata.virtual_token_reserves, "virtual token reserves");
  const realTokens = reserveTokens(metadata.real_token_reserves, "real token reserves");
  let expectedTokens = netSol > 0n ? (netSol * virtualTokens) / (virtualSol + netSol) : 0n;
  if (expectedTokens > realTokens) expectedTokens = realTokens;
  if (expectedTokens <= 0n) throw new Error("local exact-SOL quote produced zero tokens");
  const slippage = BigInt(Math.max(0, Math.min(9_000, Number(request.slippage_bps || 0))));
  const minTokens = (expectedTokens * (10_000n - slippage)) / 10_000n;
  if (minTokens <= 0n) throw new Error("local exact-SOL quote produced zero minimum output");
  return {spendableSol, expectedTokens, minTokens};
}
...
async function buildPumpLocal(request) {
  ...
  let builderMode = "official-local-pump-sdk";

  if (String(request.side).toUpperCase() === "BUY") {
    if (metadata.e4_direct_copy) {
      if (typeof PUMP_SDK.buyExactSolInInstruction !== "function") {
        throw new Error("installed Pump SDK does not expose buyExactSolInInstruction");
      }
      const quote = quoteExactSolIn(request, metadata);
      const associatedUser = getAssociatedTokenAddressSync(mint, user, false, tokenProgram);
      instructions.push(
        createAssociatedTokenAccountIdempotentInstruction(
          user,
          associatedUser,
      ...
      instructions.push(await PUMP_SDK.buyExactSolInInstruction({
        user,
        mint,
        creator,
        feeRecipient,
        solAmount: new BN(quote.spendableSol.toString()),
        minTokenAmount: new BN(quote.minTokens.toString()),
        tokenProgram,
      }));
      builderMode = "official-local-pump-sdk-exact-sol-in";
    } else {
...
function selfTestRequest(directCopy = false) {
  return {
    request_id: directCopy ? "direct-copy-self-test" : "local-self-test",
    side: "BUY",
    mint: "3hCyCV1JhuF6Rup98djLbh1fyKxHyQjTcTGEQcA1pump",
    public_key: "E4EzXdwf7NNdqM2XGswWaWHfxgucVCo24PTCcrimTKBz",
    amount: 0.01,
    denominated_in_sol: true,
    slippage_bps: directCopy ? 9000 : 800,
    priority_fee_sol: 0,
    tip_sol: 0,
    pool: "pump",
    metadata: {
      ...
      e4_direct_copy: directCopy,
    },
  };
}

async function selfTest(directCopy = false) {
  const result = await buildPumpLocal(selfTestRequest(directCopy));
...
if (process.argv.includes("--self-test-direct-copy")) {
  await selfTest(true);
  process.exit(0);
}

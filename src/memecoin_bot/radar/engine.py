...
class RadarEngine:
    ...
    def evaluate(self, current: MarketSnapshot, previous: list[dict],
                 first_discovered_at: str, basic_safety_passed: bool) -> RadarResult:
        ...
        if volume_mc is not None and volume_mc >= .15:
        ...
        if current.liquidity_usd is not None and current.liquidity_usd >= self.settings.radar_min_liquidity_usd:
            score += 10
        ...
        if age_minutes > self.settings.radar_max_age_minutes:
            ...
            score -= 30
        if current.market_cap_usd is None or current.market_cap_usd > self.settings.radar_max_market_cap_usd:
            penalties.append("ABOVE_RADAR_MARKET_CAP_RANGE")
            score -= 30
        if current.liquidity_usd is None or current.liquidity_usd < self.settings.radar_min_liquidity_usd:
            penalties.append("RADAR_LIQUIDITY_TOO_LOW")
            score -= 30
        if current.price_change_5m is not None and current.price_change_5m >= self.settings.radar_late_pump_price_change_percent:
            penalties.append("LATE_VERTICAL_PRICE_MOVE")
            score -= 45
        if liquidity_velocity is not None and liquidity_velocity < .85:
            penalties.append("LIQUIDITY_COLLAPSING")
            score -= 35
        if buy_sell is not None and buy_sell < .8:
        ...
        triggered = (
            ...
            and not {"OUTSIDE_EARLY_WINDOW", "ABOVE_RADAR_MARKET_CAP_RANGE", "RADAR_LIQUIDITY_TOO_LOW",
                     "LATE_VERTICAL_PRICE_MOVE", "LIQUIDITY_COLLAPSING", "SELL_PRESSURE_DOMINANT"} & set(penalties)
        )

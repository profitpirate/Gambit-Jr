...
def _exit_copy_fidelity_v12(self: Any, position: Any, state: Any):
    """Direct E4 copies mirror E4 exits; Gambit strategy exits cannot front-run them.

    The prior V12 path mixed E4 cumulative sells with Gambit's own adverse-markout
    and flow-break exits. Because a copied fill can be higher than E4's fill, the
    same market price can be profitable for E4 but look like a fast failure from
    Gambit's entry basis. That caused V12 to liquidate before E4's later partials
    and final exit.

    For a recognized E4 copy, E4 is therefore authoritative for normal lifecycle
    decisions. The only independent fallback is a source-stale safety exit.
    """
    ...
    # No new E4 sell has been observed. Do not let a worse copied entry basis
    # manufacture an independent stop that E4 itself never took.
...
# AllenHark's public sniper/relay architecture highlights two transport choices
# that are useful independent of its proprietary source: keep connections warm
# and fan the same signed transaction out simultaneously. V12 applies those
# properties to its existing routes, and optionally to AllenHark's documented
# REST relay when explicitly configured.
...
class FastPersistentRouteSender(_PREVIOUS_ROUTE_SENDER):
    ...
    async def _send(
        ...
        # Intentionally ignore route index: all copies of the same signed
        # transaction are dispatched simultaneously rather than staggered.
        del index
        ...
        try:
            ...
            async with session.post(
                ...
                if returned and expected_signature and returned != expected_signature:
                    # Some RPC/relay responses use request IDs instead of a
                    # signature; only reject values that look like a Solana sig.
                    if len(returned) >= 64:
                        raise RuntimeError(

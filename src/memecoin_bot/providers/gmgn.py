...
class GmgnProvider:
    ...
    async def _enrich_uncached(self, chain: str, address: str) -> GmgnSnapshot:
        ...
        async with self._semaphore:
            values = await asyncio.gather(
                self._get("info", chain, address),
                self._get("security", chain, address),
                self._get("pool", chain, address),
                self._get("holders", chain, address, 100),
                self._get("traders", chain, address, 100),
            ...
            if isinstance(value, Exception):
                ...
                if isinstance(value, dict):
                    value = (
                        value.get("list")
                        or value.get("items")
                        or value.get("holders")
                        or value.get("traders")
                    )
    ...
    def redacted_config(self) -> dict[str, Any]:
        return {
            ...
            "authentication": "X-APIKEY (redacted)",
            "mode": "READ_ONLY",
        }

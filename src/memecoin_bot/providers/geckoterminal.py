...
class GeckoTerminalDiscoveryProvider:
    ...
    async def discover(self) -> list[DiscoveryEvent]:
        ...
        included = {
            str(item.get("id")): item for item in data.get("included", [])
        ...
        for pool in (data.get("data", []) if isinstance(data, dict) else []):
            ...
            events.append(DiscoveryEvent(
                token_address=str(address), chain=self.chain, symbol=base.get("symbol"),
                name=base.get("name"), source=self.name, discovered_at=iso(),
                estimated_creation_timestamp=attrs.get("pool_created_at"),
                pair_address=attrs.get("address"), metadata={
                    "description": None, "pool_name": attrs.get("name"),
                    "pool_created_at": attrs.get("pool_created_at"), "source_pool_id": pool.get("id"),
                },
            ))
        return events

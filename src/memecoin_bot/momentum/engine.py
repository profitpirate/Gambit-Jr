...
class MomentumEngine:
    def assess(self, current: MarketSnapshot, previous: dict | None) -> dict:
        ...
        if not previous:
            return {
                "score": None, "buy_sell_ratio": buy_sell, "volume_to_market_cap": volume_mc,
        ...
        if mc_velocity is None or volume_velocity is None or buy_sell is None:
            return {"score": None, "market_cap_velocity": mc_velocity,
                    "volume_velocity": volume_velocity, "buy_sell_ratio": buy_sell,
        ...
        return {"score": round(max(0.0, min(15.0, score)), 2),
                "market_cap_velocity": mc_velocity, "volume_velocity": volume_velocity,
                "buy_sell_ratio": buy_sell, "volume_to_market_cap": volume_mc}

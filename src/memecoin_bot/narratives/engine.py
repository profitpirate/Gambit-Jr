...
CATEGORIES: dict[str, set[str]] = {
    ...
    "ANIMAL": {"dog", "cat", "frog", "penguin", "monkey", "goat", "猫", "猫猫", "ねこ", "고양이", "柴犬"},
    "GAMING": {"game", "gaming", "play"},
    "POLITICS": {"president", "election", "senate", "politics"},
    "SPORTS": {"football", "soccer", "basketball", "ufc", "sport"},
    "ASIA": {"china", "chinese", "korea", "korean", "japan", "japanese", "中国", "韩国", "日本", "한국"},
}
...
class NarrativeEngine:
    def assess(self, discovery: DiscoveryEvent, market: MarketSnapshot) -> dict[str, Any]:
        ...
        matches = [
            ...
            if words & terms or any(term in text for term in terms if any(ord(c) > 127 for c in term))
        ]
        if not matches:
        ...
        return {
            ...
            "fit_evidence": sorted(
                ...
                {term for term in CATEGORIES[matches[0]] if any(ord(c) > 127 for c in term) and term in text}
            ),
            "freshness": None,
            "acceleration": None,

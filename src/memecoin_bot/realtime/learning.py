...
def wilson_interval(successes: int, sample: int, z: float = 1.959963984540054) -> list[float] | None:
    ...
    margin = z * math.sqrt(
        proportion * (1 - proportion) / sample + z * z / (4 * sample * sample)
    ) / denominator
...
def fixed_frequency_metrics(
    ...
    metrics: dict[str, Any] = {
        ...
        "20x_recall": _rate([row in selected for row in rows if float(row.get("peak_multiple") or 0) >= 20]),
        "50x_recall": _rate([row in selected for row in rows if float(row.get("peak_multiple") or 0) >= 50]),
        "terminal_failure": _rate([bool(row.get("terminal_failure")) for row in selected]),
        "median_mae": _median(
            [float(row["maximum_adverse_excursion"]) for row in selected if row.get("maximum_adverse_excursion") is not None]
        ),
        "median_entry_mc": _median(
            [float(row["entry_market_cap"]) for row in selected if row.get("entry_market_cap") is not None]
        ),
        "median_latency_seconds": _median(
            [float(row["latency_seconds"]) for row in selected if row.get("latency_seconds") is not None]
        ),
    ...
    metrics["natural_prevalence"] = {
        target: _rate([peak >= target for peak in universe_peaks])
...
class AdaptiveLearningLab:
    ...
    def rows_from_store(self) -> list[dict[str, Any]]:
        ...
        rows = self.store.conn.execute(
            "SELECT o.token_id,o.max_multiple_from_discovery,o.final_lifecycle_state,o.updated_at,"
            "c.id candidate_id,c.first_discovered_at,c.initial_market_cap_usd,c.initial_liquidity_usd,"
            "c.normalized_score,c.classification,s.max_drawdown FROM token_outcomes o "
            "JOIN candidates c ON c.token_id=o.token_id LEFT JOIN signals s ON s.id=c.signal_id "
            "WHERE o.max_multiple_from_discovery IS NOT NULL AND o.updated_at IS NOT NULL "
            "ORDER BY c.first_discovered_at,o.token_id"
        )
        ...
        for row in rows:
            v3 = self.store.conn.execute(
                ...
                "AND available_evidence_timestamp<=decision_timestamp "
                "ORDER BY decision_timestamp LIMIT 1",
                (row["token_id"],),
            ).fetchone()
            if not v3:
                continue
            decision_at = str(v3["decision_timestamp"])
            ...
            v3_payload = json.loads(v3["v3_decision_json"])
            control_payload = json.loads(v3["control_decision_json"])
            latency = json.loads(v3["latency_json"])
            peak = float(row["max_multiple_from_discovery"])
            terminal = str(row["final_lifecycle_state"] or "").startswith("FAILED")
            output.append(
                {
                    "entity_key": f"token:{row['token_id']}",
                    "decision_at": decision_at,
                    "outcome_available_at": str(row["updated_at"]),
                    "peak_multiple": peak,
                    "terminal_failure": terminal or peak < 0.3,
                    "maximum_adverse_excursion": row["max_drawdown"],
                    "entry_market_cap": row["initial_market_cap_usd"],
                    "latency_seconds": latency.get("provider_seconds"),
                    "copyable": bool(
                        row["initial_market_cap_usd"] and row["initial_liquidity_usd"]
                    ),
                    "stage": (feature.get("migration_state") or "UNKNOWN"),
                    "features": _flatten_numeric(feature),
                    "control_score": row["normalized_score"],
                    "v3_score": v3_payload.get("expected_utility"),
                    "control_selected": str(row["classification"] or "")
                    in {"WATCH", "STRONG", "HIGH_CONVICTION"},
                    "control_components": (
                        control_payload.get("legacy", {}).get("component_scores") or {}
                    ),
    ...
    def run_store(self, model_name: str = "REALTIME_TRAJECTORY_CHALLENGER_V1") -> dict[str, Any]:
        ...
        validation_end = (
            max(_timestamp(str(row["outcome_available_at"])) for row in rows)
            + datetime.resolution
        ).isoformat()
        return self.run(
            rows,
    ...
    def chronological_split(
        ...
        validation = [
            row
            for row in ordered
            if dev_end <= _timestamp(str(row["decision_at"])) < valid_end
        ]
    ...
    def run(
        ...
        *,
        ...
        development, validation = self.chronological_split(
            rows, development_end, validation_end
        )
        ...
        for row in validation:
            ...
            candidate = row.get("realtime_candidate_score")
            failure = float((row.get("features") or {}).get("failure_probability") or 0)
            row["control_v3_hybrid_score"] = (
                (float(control) + float(candidate)) / 2 - failure
                if isinstance(control, (int, float)) and isinstance(candidate, (int, float))
                else None
            )
        frontiers: dict[str, dict[str, Any]] = {}
        for score_key in ("control_score", "v3_score", "realtime_candidate_score", "control_v3_hybrid_score"):
            frontiers[score_key] = {
        ...
        with self.store._lock, self.store.conn:
            self.store.conn.execute(
                ...
                (
                    ...
                    _json({"start": development_end, "end": validation_end, "sample": len(validation)}),
    ...
    def low_performance_autopsy(
        ...
        categories = {
            "WASH_OR_RECYCLE": lambda row: float((row.get("features") or {}).get("wash_probability") or 0) >= 0.3,
            "LINKED_WALLETS": lambda row: float((row.get("features") or {}).get("linked_wallet_share") or 0) >= 0.3,
            "CREATOR_LINKED_FLOW": lambda row: float((row.get("features") or {}).get("creator_linked_share") or 0) >= 0.25,
            "CAPITAL_REVERSAL": lambda row: bool((row.get("features") or {}).get("capital_reversal")),
            ...
            "LOW_EVIDENCE_COVERAGE": lambda row: float((row.get("features") or {}).get("evidence_coverage") or 0) < 0.6,
        }
        roots = []
                ...
                roots.append(
                    {
                        ...
                        "false_positive_rate": fp_hits / len(false_positives) if false_positives else None,
    ...
    def _matched_differences(
        ...
        for winner in winners:
            candidates = [
                ...
                and abs(float(row.get("entry_market_cap") or 0) - float(winner.get("entry_market_cap") or 0))
            ...
            candidates.sort(
                key=lambda row: abs(
                    (_timestamp(str(row["decision_at"])) - _timestamp(str(winner["decision_at"]))).total_seconds()
                )
            )
            other = candidates[0]
        ...
        return [
            {"feature": name, "matched_pairs": len(values), "median_difference": statistics.median(values)}
    ...
    def control_autopsy(rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        cohorts = {
            ...
            "FALSE_POSITIVE": [row for row in selected if float(row.get("peak_multiple") or 0) < 2 and not row.get("terminal_failure")],
            "TERMINAL_FAILURE": [row for row in selected if row.get("terminal_failure")],
            "RIGHT_TAIL_RUNNER": [row for row in selected if float(row.get("peak_multiple") or 0) >= 10],
        }
    ...
    def stage_b_autopsy(rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "true_positives_removed": sum(float(row.get("peak_multiple") or 0) >= 2 for row in removed),
            "false_positives_retained": sum(float(row.get("peak_multiple") or 0) < 2 for row in retained),
    ...
    def discover_hypotheses(
        ...
        for name, spec in sorted(
            ...
            same_direction = validation_effect is not None and spec["effect"] * validation_effect > 0
            ...
            self.store.conn.execute(
                ...
                (
                    ...
                    "MEDIUM",
                    _json([name]),
                    _json({"development": record["development_sample"], "validation": record["validation_sample"]}),

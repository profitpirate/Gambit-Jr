#!/usr/bin/env python3
"""Validate and atomically register one frozen conformal live capture."""
...
import e4_v12_online_conformal_holdout as holdout
import e4_v12_profit_survival_search as base

SCHEMA_VERSION = "e4-v12-online-conformal-capture-registration-v1"
BATCH_NAME = "e4-v12-online-conformal-forward-batch.json"
EVENTS_NAME = "e4-v12-online-conformal-forward-batch-live-events.jsonl"
...
def capture_record(
    ...
    batch_path = artifact_file(artifact_dir, BATCH_NAME)
    events_path = artifact_file(artifact_dir, EVENTS_NAME)
    ...
    launches = int(capture.get("unique_launches", 0))
    if launches != required or len(cohort) != required:
    ...
    if capture.get("target_reached") is not True:
    ...
    return {
        ...
        "artifact_name": f"e4-v12-online-conformal-forward-{run_id}",
        "workflow_conclusion": "success",
        "workflow_run_started_at": workflow_started_at,
        "workflow_run_started_at_epoch_ns": epoch_ns(workflow_started_at),
...
def register(
    ...
    candidate["captures"] = holdout.validate_manifest(root, candidate, protocol)
...
def main() -> None:
    ...
    record = capture_record(
        root, artifact_dir, str(args.run_id), args.workflow_started_at, protocol
    )

...
import e4_v12_funder_cluster_search as search
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply a frozen causal upstream-funder cluster thesis"
    )
    ...
    if frozen.get("status") != "HISTORICAL_GOLDEN_CONFIRMED":
        raise SystemExit("funder-cluster thesis was not historically confirmed")
    ...
    relations = search.load_relations(args.intelligence_cache)
    snapshots = search.build_snapshots(runs, relations)
    live_index = len(runs) - 1
    live_rows = [
        row
        for row in snapshots
        if search.integer(row.get("run_index")) == live_index
    ]
    predictions = search.select(live_rows, rule)
    run_map = {run.run_id: run for run in runs}
    latencies = [
        search.finite(value)
        for value in args.latencies_ms.split(",")
        if value.strip()
    ]
    ...
    cache = json.loads(args.intelligence_cache.read_text(encoding="utf-8"))
    payload = {
        "version": "e4-v12-funder-cluster-live-v1",
        "status": (
            "FRESH_LIVE_GOLDEN_CONFIRMED" if passed else "NOT_CONCLUSIVE"
        ),
        "live_run": runs[-1].run_id,
        "live_snapshots": len(live_rows),
        "relation_count": len(relations),
        "intelligence_cutoff_ns": cache.get("cutoff_ns"),

...
import e4_v12_reactive_confidence_search as search
import e4_v12_reactive_output_guard_search as source
import e4_v12_true_latency_replay as replay


def main() -> int:
    parser = argparse.ArgumentParser(description="Apply frozen E4 stake-confidence/output-guard thesis")
    ...
    if frozen.get("status") != "HISTORICAL_GOLDEN_CONFIRMED":
        raise SystemExit("reactive-confidence thesis was not historically confirmed")
    ...
    candidates = source.build_candidates(runs)
    ...
    payload = {
        "version": "e4-v12-reactive-confidence-live-v1",

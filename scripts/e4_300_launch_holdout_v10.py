...
def load_holdout():
    ...
    name = "e4_300_launch_holdout_v10_base"
...
if __name__ == "__main__":
    ...
    def anchor_to_live_v10(item, *args, **kwargs):
        ...
        if event is None:
            return None
        # Native live events bypass Event.from_row, so project the decoded
        # Anchor payload into the same creator/social/learning caches here.
        v10.v8.observe_context(event.mint, item)
        ...
        if event.kind == base.core.EventKind.CREATE.value:
            v10.PIPELINES.observe_launch_event(
                mint=event.mint,
            ...
            v10.PIPELINES.observe_trade_event(
                mint=event.mint,
                received_ns=event.received_ns,
                price_sol=event.price_sol or 0.0,

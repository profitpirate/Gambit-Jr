...
def command_python(*parts: str) -> list[str]:
    return [sys.executable, *parts]


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Run all three Gambit E4 V10 pipelines")
    ...
    value.add_argument("--live", action="store_true", help="Pass --live to the funded execution process")
...
def main() -> int:
    ...
    execution = command_python("-m", "memecoin_bot.e4_exec_v10")
    if args.live:
        execution.append("--live")
    spawn("execution", execution)
    if args.social:
        spawn(
            "social",
            command_python(
                ...
                "--config",
                str(args.social_config),
            ),
        )
    if args.learner:
        spawn("learner", command_python("scripts/e4_creator_learner_v10.py"))
    ...
    while not stopping:
        for name, child in tuple(children.items()):
            ...
            if name == "social":
                spawn(name, command_python("scripts/e4_social_stream_v10.py", "--config", str(args.social_config)))
            elif name == "learner":
                spawn(name, command_python("scripts/e4_creator_learner_v10.py"))
        time.sleep(0.1)
    ...
    for child in children.values():
        if child.poll() is None:
            try:
                ...
                child.kill()
    return children.get("execution").returncode if children.get("execution") else 1

#!/usr/bin/env python3
"""Record the retired immediate-and-300 ms stable-profit experiment."""
...
OUTPUT = Path("artifacts/e4-v12-latency-stable-profit-failure.json")
SCHEMA_VERSION = "e4-v12-latency-stable-profit-failure-v1"
THESIS_FAMILY = "latency-stable-profit-v12"
MODEL_PARAMETERS = {
    "learning_rate": 0.05,
    "max_iter": 100,
    "max_leaf_nodes": 15,
...
def main() -> dict[str, Any]:
    ...
    pnl_300 = np.load(labels.PNL_PATH, allow_pickle=False)
    immediate_matrix = np.load(
        conformal.PNL_MATRIX, mmap_mode="r", allow_pickle=False
    )
    pnl_immediate = immediate_matrix[:, conformal.POLICY_INDEX]
    if len(pnl_immediate) != len(pnl_300) or len(pnl_300) != len(rows):
        raise ValueError("row-aligned PnL labels changed")
    if not bool(np.all(np.isfinite(pnl_immediate))):
        raise ValueError("immediate labels are incomplete")
    stable_profit = (pnl_immediate > 0) & (pnl_300 > 0)
    label_audit = {
        "rows": len(rows),
        "immediate_matrix_shape": list(immediate_matrix.shape),
        "immediate_matrix_sha256": base.sha256_path(conformal.PNL_MATRIX),
        "latency300_pnl_sha256": metadata["pnl_sha256"],
        "immediate_positive": int(np.sum(pnl_immediate > 0)),
        "latency300_positive": int(np.sum(pnl_300 > 0)),
        "both_positive": int(np.sum(stable_profit)),
        "immediate_only": int(np.sum((pnl_immediate > 0) & (pnl_300 <= 0))),
        "latency300_only": int(np.sum((pnl_immediate <= 0) & (pnl_300 > 0))),
    ...
    for fold_number, (test_start, calibration_size, test_size) in enumerate(
        ...
        model.fit(features[fit], stable_profit[fit])
        ...
        result = conformal.replay(rows, test[mask], pnl_300, test_scores[mask])
        result.update(
            {
                "fold": fold_number,
    ...
    identity = {
        ...
        "model_family": "latency_stable_histogram_boosting",
        "full_parameters": {
            "model": MODEL_PARAMETERS,
            "random_seed_base": RANDOM_SEED_BASE,
            "target": "pnl_immediate > 0 and pnl_300ms > 0",
            "immediate_matrix_sha256": label_audit["immediate_matrix_sha256"],
            "latency300_label_sha256": metadata["pnl_sha256"],
        ...
        "candidate_risk_set_policy": (
            "fit joint immediate-and-300ms profit stability; score at 250ms; "
            "accept above the causal rolling 99.73rd percentile"
        ),
        ...
        "latency_assumptions_ms": [0, labels.LATENCY_MS],
        "execution_policy": (
    ...
    output = {
        ...
        "scientific_basis": (
            "A fixed joint target tested whether excluding immediate-entry "
            "winners that fail at 300 ms improves realistic-latency precision."
        ),
        "aggregate": summary,
        ...
        "failure_reason": (
            "The joint target stayed profitable but reached only 59.16% WR "
            "and 53.80% Wilson; the final fold fell to 50.54% WR with four "
            "profitable windows, below the frozen precision and breadth gates."
        ),
        "material_change_required_before_rerun": (
            "A genuinely new causal feature or joint model family; Boolean "
            "target, latency-window, threshold, and parameter variations are prohibited."
        ),
        "anti_lookahead": {

#!/usr/bin/env python3
"""Record the retired 300 ms latency-aware conformal experiment."""
...
OUTPUT = Path("artifacts/e4-v12-latency300-conformal-failure.json")
SCHEMA_VERSION = "e4-v12-latency300-conformal-failure-v1"
THESIS_FAMILY = "runtime-latency-aware-conformal-v12"
MODEL_PARAMETERS = {
    "learning_rate": 0.05,
    "max_iter": 100,
    "max_leaf_nodes": 15,
...
def main() -> dict[str, Any]:
    ...
    requirements = {
        ...
        "full_quote_coverage": label_metadata["no_quote_rows"] == 0,
    }
    chronology = [
        {
    ...
    identity = {
        ...
        "model_family": "histogram_gradient_boosted_latency_profit_classifier",
        "full_parameters": {
        ...
        "candidate_risk_set_policy": (
            "fit 300 ms net-profit labels; score at 250 ms; accept above the "
            "causal rolling 99.73rd percentile of prior scores"
        ),
    ...
    output = {
        ...
        "identity": identity,
        "scientific_basis": (
            "An independent 147,000-launch audit measured 297.207515 ms median "
            "source-to-fill latency, so labels were rebuilt at a fixed 300 ms."
        ),
        ...
        "failure_reason": (
            "Latency-aware training improved the 300 ms baseline but reached "
            "only 61.04% WR and a 55.65% Wilson lower bound, below the frozen "
            "65% and 60% development requirements."
        ),

...
import e4_v12_social_whitelist_search as combined
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if frozen.get("status") != "HISTORICAL_GOLDEN_CONFIRMED":
        raise SystemExit("social-whitelist thesis was not historically confirmed")
    rule = social.Rule(**frozen["rule"])
    ...
    combined.seed_prior_history(snapshots, registry)
    live_index = len(runs) - 1
    live_rows = [row for row in snapshots if social.integer(row.get("run_index")) == live_index]
    predictions = social.select(live_rows, rule)
    ...
    payload = {
        "version": "e4-v12-social-whitelist-live-v1",
        ...
        "rule": rule.as_dict(),
        "prior_registry_ref": registry.get("causal_ref"),
        "economics": social.compact(economics),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply frozen prior-whitelist plus prelaunch-social thesis")

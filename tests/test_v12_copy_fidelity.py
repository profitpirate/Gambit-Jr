...
class V12CopyFidelityTests(unittest.TestCase):
    ...
    def test_token_accounted_multileg_exit_is_not_collapsed_after_second_sell(self):
        ...
        observed = time.time_ns()
        self._install_direct(mint, self._source(mint, observed))
        first = PIPELINES.observe_e4_exit(mint, token_amount=300.0, observed_ns=observed + 1_000_000, signature="sell-1")
        self.assertFalse(first.fully_exited)
        self.assertAlmostEqual(first.remaining_tokens, 700.0)
        second = PIPELINES.observe_e4_exit(mint, token_amount=350.0, observed_ns=observed + 2_000_000, signature="sell-2")
        self.assertFalse(second.fully_exited)
        self.assertAlmostEqual(second.remaining_tokens, 350.0)
        third = PIPELINES.observe_e4_exit(mint, token_amount=200.0, observed_ns=observed + 3_000_000, signature="sell-3")
        self.assertFalse(third.fully_exited)
        self.assertAlmostEqual(third.remaining_tokens, 150.0)
        fourth = PIPELINES.observe_e4_exit(mint, token_amount=150.0, observed_ns=observed + 4_000_000, signature="sell-4")
    ...
    def test_e4_full_exit_remains_authoritative(self):
        ...
        action, fraction, reason = self._policy().exit(
            self._position(mint), self._state(mint, price=1.10, latest_ns=observed + 2_000_000)
        )
    ...
    def test_source_staleness_is_the_independent_fail_safe(self):
        ...
        self._install_direct(mint, self._source(mint, observed))
        action, fraction, reason = self._policy().exit(
            self._position(mint), self._state(mint, price=1.0, latest_ns=observed + 66_000_000_000)
        )
        self.assertEqual(action, "SELL_ALL", reason)
        self.assertEqual(fraction, 1.0)
    ...
    def test_copy_fidelity_patch_is_active(self):

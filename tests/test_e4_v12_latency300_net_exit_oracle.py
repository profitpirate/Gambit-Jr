...
diagnostic = load_module(
    "e4_v12_latency300_net_exit_oracle_tests",
...
def report() -> dict:
    return json.loads(
        (ROOT / "artifacts/e4-v12-latency300-net-exit-oracle.json").read_text(
            encoding="utf-8"
        )
    )


def test_oracle_is_bound_to_exact_frozen_risk_set_and_policy_grid() -> None:
    result = report()
...
def test_no_fee_net_policy_can_repair_final_chronological_fold() -> None:
    result = report()
    assert [fold["feasible_policies"] for fold in result["folds"]] == [11, 16, 14, 0]
    best = result["final_fold_oracle_best"]
    assert best["win_rate"] == 0.5375
    assert best["net_pnl_sol"] < -0.25
    assert best["profit_factor"] < 0.53
...
def test_oracle_never_consumes_live_or_authorises_production() -> None:
    result = report()
